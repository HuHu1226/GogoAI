...
def make_product_container(product_name, product_info, image_height, each_card_offset):
    ...
    with st.container(border=True, height=image_height + each_card_offset):
        ...
        with image_col:
            ...
            image = resize_image(product_info["images"], max_height=500)
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown(f"共有街道：{product_num} 件")
        st.markdown(f"共有街道介绍：{product_num} 个")
    ...
    with st.form(key="add_product_form"):
        product_name_input = st.text_input(label="添加街道名称")
        ...
        product_image = st.file_uploader(label="上传街道图片", type=["png", "jpg", "jpeg", "bmp"])
        product_instruction = st.file_uploader(label="上传街道详细介绍", type=["md"])
...
def update_product_info(
    ...
    参数:
    - product_name_input: 地点名称输入，字符串类型。
    - heightlight_input: 街道主要特点，字符串类型。
    - product_image: 地点图片，图像类型。
    - product_instruction: 地点介绍，文本类型。
    - departure_place: 发货地。
